...
>
    {#each options as opt (opt.value)}
        <RadioGroup.Item
            value={opt.value}
            class={
                'group flex items-center gap-2.5 w-full text-left text-sm text-textcolor ' +
        ...
        >
            <span
                class="relative shrink-0 size-4 rounded-full border border-darkborderc transition-colors group-data-[state=checked]:border-primary"
            >
                <span
